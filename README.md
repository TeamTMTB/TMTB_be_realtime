...
아키텍처는 기본적으로 docker와 eks를 통한 컨테이너 기반의 아키텍처 입니다. <br>
MSA구조 아래서 도메인별로 나누어진 서비스 별로 각각의 서버들이 Pod로 띄워져 있습니다. <br>
외부 접속에대한 라우팅은 쿠버네티스의 인그레스 기능을 활용하여 라우팅하였습니다. 또한 프론트엔드는 Nginx에 올려 좀더 빠른 페이지전달이 가능하도록 했습니다.<br>
RealTime Service의 경우 실시간 채팅을 관리하는 서버인데, 각각 pod로 나누어진 서버들이 공통적으로 사용자의 세션정보를 가지고 있어야하므로 redis를 통해 그 정보를 공유할수 있게 하였습니다.<br>
